...
def _build_resume_analysis_prompt(state: ResumeAnalysisRequest, json_only: bool = False) -> str:
    prompt = f"""
{SYSTEM_PROMPT_FOR_RESUME_ANALYSIS}
...
def _generate_resume_analysis_response(prompt: str) -> ResumeAnalysisBundleResponse:
    ...
    try:
        ...
        logger.warning("Resume analysis structured output failed; falling back to raw JSON mode.", exc_info=True)
    ...
    try:
        ...
        logger.warning("Resume analysis raw output parsing failed: %s", exc, exc_info=True)
