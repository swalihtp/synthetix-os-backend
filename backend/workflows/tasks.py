...
def process_gmail_notification_task(self, state):
    try:
        gmail_notification_app.invoke(state)
        return "NOTIFICATION HANDOVER TO EMAIL WORKFLOW"
...
def ingest_documents_to_vector_db(
    ...
    try:
        for document in documents:
            document_key = document.document.name
            logger.info(
                "[ingest_documents_to_vector_db] document_id=%s file_path=%s",
                document.id,
                document_key,
            )
            extracted = load_documents([document_key])

            if not extracted:
                raise ValueError(
                    f"No extractable content found for document '{document_key}'"
                )
...
def process_resume_analysis(self, file_path, execution_id, job_title, job_description):
    ...
    try:
        logger.info(
            "[process_resume_analysis] file_path=%s execution_id=%s",
            file_path,
            execution_id,
        )
...
def process_meeting_notes_analysis(self, file_path, execution_id, summary_style):
    ...
    try:
        logger.info(
            "[process_meeting_notes_analysis] file_path=%s execution_id=%s",
            file_path,
            execution_id,
        )
        workflow_execution = WorkflowExecution.objects.get(id=execution_id)
